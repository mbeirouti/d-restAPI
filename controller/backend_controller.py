...
from controller.helper_functions import validate_patient_data, plug_patient_data_into_json, \
    plug_patients_into_page_json, plug_http_error_into_json
...
def add_patient_to_database(patient_data):
    ...
    if data_is_valid[0]:
        ...
        database_response = cursor.execute("SELECT * FROM patients WHERE email=?",
...
def get_patients_page(page_number):
    ...
    database_response = cursor.execute("SELECT * FROM patients ORDER BY rowid LIMIT ? OFFSET ?",
                                       [patients_per_page, (page_number - 1) * 10])
